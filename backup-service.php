...
define(SEND_EMAIL, 'info@example.com');
...
define(DATETIMEFORMATFORFILE, 'd-m-y-h-i-s');

// START PROCESS

(new backup)
->filename( "SlavaNadejdin" )
->db()
->addTime()
...
class backup{
    ...
    private function send_( $files = false ){
        ...
        $mail->Port = SMTP_PORT;                                    
        $mail->setFrom(SEND_EMAIL, 'Backup Service');
        ...
        $mail->isHTML(true);     
        $mail->Subject = 'Backup - ' . date(DATETIMEFORMAT);
        $mail->Body    = 'Backup - ' . date(DATETIMEFORMAT);
        $mail->AltBody = 'Backup - ' . date(DATETIMEFORMAT);
        if(!$mail->send()) {
            return false;
        } else {
            return true;
    ...
    public function db(){
        $this->db_file = backup_tables(DBHOST,DBUSER,DBPASSWD,DBNAME);
        return $this;
    }
    ...
    public function zip(){
        $zip = new Zip();
        $zip->setComment("Zip backup file. \nCreated on " . date(DATETIMEFORMAT));
        ...
        $this->filename = (($this->addTime==true)?date(DATETIMEFORMATFORFILE)."-":"").$this->filename;
